...
def xyxy2xywh_wide(x):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] where xy1=top-left, xy2=bottom-right
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = (x[:, 0] + x[:, 2]) / 2  # x center
    y[:, 1] = x[:,3]-(x[:,3]-x[:,1])*0.1 # y center
    y[:, 2] = (x[:, 2] - x[:, 0]) *1.8 # width
    y[:, 3] = (x[:, 3] - x[:, 1]) *2 # height
    return y

def xyxy_wide2xyxy_narrow(x):
    xc=(x[0]+x[2])/2
    yc=(x[1]+x[3])/2
    w=(x[2]-x[0])
    h=(x[3]-x[1])
...
def non_max_suppression_face(prediction, conf_thres=0.45, iou_thres=0.45, classes=None, agnostic=False, labels=()):
